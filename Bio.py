...
class BioMod(loader.Module):
    """
Modify by @caesar_do_not_touch for @caesar_do_not_touch 
    """
    strings = {
        ...
        "r.save":   
            ...
            "<b><emoji document_id=5278372097905664675>🧬</emoji> +{}{}</b> био-ресурсов",
        "auto.save":   
            "<emoji document_id=5212932275376759608>💘</emoji> Жертва <b><code>{}</code></b> в зарлисте.\n"
            "<b><emoji document_id=5278372097905664675>🧬</emoji> {}+{}</b> био-ресурсов.",        
        "search":
            "<emoji document_id=5212932275376759608>💓</emoji> Жертва <code>{}</code> приносит:\n"
            "<b><emoji document_id=5278372097905664675>🧬</emoji> +{} био-ресурсов.</b>\n"
            "📆 Дата: <i>{}</i>",
        ...
        "no_sargs": "<emoji document_id=5215273032553078755>💔</emoji> Не найдено совпадение в на����але строк с аргументами.",
            ...
            if re.fullmatch(r"лаб[ау]{,2}", toxt, flags=re.ASCII): # регулярка
                ...
                for i in lab_lines: # цикл for по всем строкам в тексте лабы
                    if "🔬 Досье лаборатории " in i:
                        s = i.replace("🔬 Досье лаборатории ", "")
                        sms += f"🔬 Досье лабы \"<a href=tg://user?id={me.id}>им. Салата</a>\":\n"
                    if "🧪 Готовых патогенов: " in i:
                        s = i.replace("🧪 Готовых патогенов: ", "")
                        sms += f"🧪 Готовых патов: {s}\n"
                    if "⏱ Новый патоген: " in i:
                        s = i.replace("⏱ Новый патоген: ", "")
                        sms += f"⏱ Новый пат: {s}\n"
                    if "☣️ Био-опыт: " in i:
                        s = i.replace("☣️ Био-опыт: ", "")
                        sms += f"<b>☣️ Био-опыт: </b>{s}\n"
                    if "🧬 Био-ресурс: " in i:
                        s = i.replace("🧬 Био-ресурс: ", "")
                        sms += f"<b>🧬 Био-ресурс: </b>{s}\n"
...
#######################   
    ...
    async def зоcmd(self, message):
        """
Используй: .зо {кол-во патов} [реплай]. Покупает вакцину и бьет по сб
        """
        try:
            reply = await message.get_reply_message()
            args = utils.get_args_raw(message)
        ...
        pat = ""
        if not reply:
            return await message.edit("реплая на сб нет.")
        if args:
            try:
                pat = int(args)
            except:
